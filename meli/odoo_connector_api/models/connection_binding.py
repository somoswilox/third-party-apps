...
class OcapiConnectionBindingProductTemplate(models.Model):
    ...
    _unique_conn_id_product_tmpl = versions.UniqueIndex('connection_account, conn_id, conn_variation_id, product_tmpl_id', message='Binding exists for this item: product_tmpl_id!')
...
class OcapiConnectionBindingProductVariant(models.Model):
    ...
    product_id = fields.Many2one("product.product",string="Product Binding", help="Product Binding", index=True)

    image_bindings = fields.One2many('ocapi.product.image', "binding_product_variant_id", string="Product Variant Images")

    _unique_conn_id_variant = versions.UniqueIndex('connection_account, conn_id, conn_variation_id, product_id', message='Binding exists for this item: product_id!')
...
class OcapiConnectionBindingProductCategory(models.Model):
    ...
    _unique_conn_id_category = versions.UniqueIndex('connection_account, conn_id, conn_variation_id, category_id', message='Binding exists for this item: category!')
