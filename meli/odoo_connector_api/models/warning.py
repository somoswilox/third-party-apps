...
class warning(models.TransientModel):
    ...
    def _format_error( self, title, message, message_html='', context=None ):
        context = context or self.env.context
        return self._format_ocapi_error(title=title,message=message,message_html=message_html, context=context)
    
    def _format_ocapi_error( self, title, message, message_html='', context=None ):
        context = context or self.env.context
        ...
        if rjson:
            _logger.info("_format_ocapi_error rjson:"+str(rjson))
            ...
            if rmessage and type(rmessage)==dict:
                ...
                for rmess in rmessage:
                    ...
                    if rmess == "error":
                        ...
                        ecodemess = (ecode in ocapi_errors and ocapi_errors[ecode]) or ecode
    ...
    def warning(self, title, message, message_html='', context=None):
        context = context or self.env.context
        title, message, message_html = self._format_error(title=title,message=message,message_html=message_html,context=context)
