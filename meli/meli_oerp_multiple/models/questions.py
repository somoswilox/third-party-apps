...
class mercadolibre_questions(models.Model):
    ...
    def prepare_question_fields( self, Question, meli=None, config=None ):
        #_logger.info("prepare_question_fields:"+str(Question)+" config:"+str(config))
    ...
    def fetch_question( self, question_id=None, meli=None, config=None ):
        ...
        #_logger.info("fetch question:"+str(self)+" question_id:"+str(question_id)+" config:"+str(config))
        ...
        if question_id:
            res_q_id = "/questions/"+str(question_id)
            #_logger.info("questions res uri:"+str(res_q_id)+"?access_token="+str(meli and meli.access_token))
            response = meli.get(res_q_id, {'access_token':meli.access_token})
            if response:
