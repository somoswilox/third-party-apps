...
export class BryntumGanttController extends Component {
        ...
        onWillStart(async () => {
            ...
            try {
                ...
                if (this.props && this.props.domain.length > 0) {
                    ...
                    // projectID was an int, now is a list, modified also in Bryntum-li

                    const projectID = this.get_project_id(this.props.domain);
                    if (projectID.length === 0) {
                        // If we are launching from project.task menu, we will have no
                        // projectID, in that case do not get project id (null)
                        // from props, but leave as-is, loading last stored value
                        window.o_gantt.projectID = [];
                        window.action_from_odoo = false;
                    } else {
                        window.o_gantt.projectID = projectID;
                ...
                window.o_gantt.bryntum_css = val.bryntum_css;
                window.o_gantt.copy_dependencies = val.bryntum_copy_dependencies;
                window.o_gantt.default_calendar = val.bryntum_default_calendar;
                ...
                window.o_gantt.bryntum_odoo_colors = val.bryntum_odoo_colors;
                window.o_gantt.date_format = val.date_format;
            } catch (err) {
    ...
    async load_libs(val) {
        ...
        const r3 = await loadCSS(
            "/bryntum_gantt_enterprise/static/src/css/gantt." + val.bryntum_css + ".css"
        );
        return [r1, r2, r3];
    }
    ...
    get_project_id(propsdomain) {
        ...
        only_prj_leafs.forEach((leaf) => {
            ...
            if (leaf[1] === "in" && leaf[2].constructor === Array) {
                project_ids = project_ids.concat(leaf[2]);
            }
        });
        return project_ids;
    }
