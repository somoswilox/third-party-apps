...
class AccountPayment(models.Model):
    ...
    def update_account_move_from_account_payment(self):
        batch_active = self.batch_payment_id and self.batch_payment_id.state in ('reconciled', 'sent')
        if batch_active and self.payment_method_line_id.name == 'Detracción':
            self.tempora_accmov.write({
                'l10n_pe_detraction_number': self.detractions_constancy_number,
                'l10n_pe_detraction_date': self.batch_payment_id.date,
            })
        if batch_active and self.payment_method_line_id.name == 'Autodetracción':
            self.reference_invoice.write({
                'l10n_pe_detraction_number': self.detractions_constancy_number,
                'l10n_pe_detraction_date': self.batch_payment_id.date,
            })
...
class AccountBatchPayment(models.Model):
    ...
    payment_method_line_detraction = fields.Many2one(
        comodel_name='account.payment.method.line',
        string='Método para pago masivo',
    ...
    def _compute_filter_detraction(self):
        for rec in self:
            payment_method_line_detraction = False
            if rec.payment_ids:
                for payment in rec.payment_ids:
                    if payment.payment_method_line_id:
                        payment_method_line_detraction = payment.payment_method_line_id
            rec.payment_method_line_detraction = payment_method_line_detraction
            rec.payment_method_name = payment_method_line_detraction and payment_method_line_detraction.name
    ...
    def generate_detraction(self):
        ...
        for payment in self.payment_ids:
            if payment.currency_id.name == 'PEN':
                ...
                for acc_move in payment.move_id.line_ids:
                    if acc_move.account_id.account_type == 'asset_cash':
        ...
        if self.payment_method_name == 'Detracción':
            ...
            for obj_line in self.payment_ids:
                for line in obj_line.reconciled_bill_ids.invoice_line_ids:
                    if line.product_id.l10n_pe_withhold_code:
                        withhold_code = line.product_id.l10n_pe_withhold_code
                if not obj_line.reconciled_bill_ids:
                    raise UserError("Usted debe seleccionar una factura conciliada para que pueda generar su txt."
                                    "Verifique si tiene una factura asociada al pago respectivo")
                else:
                    ...
                    if obj_line.currency_id.name == 'PEN':
                        values = {
                            ...
                            'operation_type': obj_line.reconciled_bill_ids.operation_type_detraction if obj_line.reconciled_bill_ids.operation_type_detraction else '',
                            'invoice_date': obj_line.reconciled_bill_ids.invoice_date if obj_line.reconciled_bill_ids.invoice_date else obj_line.reference_invoice.invoice_date,
                            'document_type': obj_line.reconciled_bill_ids.l10n_latam_document_type_id.code or '01',
                            'ref': obj_line.reconciled_bill_ids.ref if obj_line.reconciled_bill_ids.ref else obj_line.memo
                        }
                        list_data.append(values)
                    else:
                        amount_not_PEN = 0
                        for acc_mov in obj_line.move_id.line_ids:
                            if acc_mov.account_id.account_type == 'asset_cash':
                                amount_not_PEN = round(acc_mov.credit or acc_mov.debit)
                                break
                        values = {
                            'ruc': obj_line.partner_id.vat,
                            'selection_ids': withhold_code,
            ...
            list_data = []
            for obj_line in self.payment_ids:
                for line in obj_line.reference_invoice.invoice_line_ids:
                    if line.product_id.l10n_pe_withhold_code:
                        withhold_code = line.product_id.l10n_pe_withhold_code
                if not obj_line.reference_invoice:
                    ...
                    if obj_line.currency_id.name == 'PEN':
                        values = {
                            ...
                            'bank': bank_nation,
                            'amount': str(obj_line.amount),
                            'operation_type': obj_line.reference_invoice.operation_type_detraction or '',
                            'invoice_date': obj_line.reference_invoice.invoice_date,
                            'document_type': obj_line.reference_invoice.l10n_latam_document_type_id.code or '01',
