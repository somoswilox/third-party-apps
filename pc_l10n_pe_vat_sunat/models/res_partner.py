...
class ResPartner(models.Model):
    ...
    is_good_taxpayer = fields.Boolean('Is Good Taxpayer')
    is_retention_agent = fields.Boolean('Is Retention Agent')

    def UnzipFileAndSearchRuc(self, vat, url):
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36"
        }
        response = requests.get(url, headers=headers)
        # Comprobar si la descarga fue exitosa (código de estado 200)
        if response.status_code == 200:
            # Leer el archivo ZIP desde la respuesta y descomprimirlo en memoria
            with ZipFile(io.BytesIO(response.content)) as zip_ref:
                # Obtener el nombre del archivo dentro del ZIP
                text_file_name = zip_ref.filelist[0].filename 
                # Obtener el contenido del archivo de texto como una sola cadena
                text_data = zip_ref.read(text_file_name).decode('iso-8859-1')
                # Dividir el contenido en líneas usando "\r" como separador
                lines = text_data.split("\r")
                # Procesar cada línea
                for line in lines:
                    line = line.strip()
                    if line:
                        # Divide la línea por el carácter "|" y obtén los campos
                        line_parts = line.split("|")
                        if len(line_parts) >= 4:
                            ruc, nombre, fecha, resolucion = line_parts[:4]
                            # Verifica si el RUC está en tu lista y realiza la acción necesaria
                            if vat == ruc:
                                response.close()
                                return True
        else:
            print("Error al descargar el archivo ZIP para obtener datos de sunat.")
        response.close()


    def CheckGoodTaxpayer(self, vat):
        url = "https://ww3.sunat.gob.pe/descarga/BueCont/BueCont_TXT.zip"
        self.is_good_taxpayer = self.UnzipFileAndSearchRuc(vat, url)

    def CheckRetentionAgent(self, vat):
        url = "https://ww1.sunat.gob.pe/descarga/AgentRet/AgenRet_TXT.zip"
        self.is_retention_agent = self.UnzipFileAndSearchRuc(vat, url)

    def update_document(self):
        for rec in self:
            rec._doc_number_change()
    ...
    def _doc_number_change(self):
        ...
        if vat and vat_type:
            if vat_type == '1':
                ...
                self.CheckGoodTaxpayer(vat)
                self.CheckRetentionAgent(vat)
    ...
    def ConsultarRepresentantesLegales(self, sesion, urlReferencia, desRuc, numeroRUC, numeroRandom):
        ...
        if(response.status_code == 200):
            ...
            if oEnSUNAT.TipoRespuesta == 1 or oEnSUNAT.MensajeRespuesta == 'No se encontro información para representantes legales.':
                if not self._origin:
        ...
        if(response.status_code == 200):
            ...
            if (oEnSUNAT.TipoRespuesta == 1):
                ...
                for i in range(1, len(state)):
                    state_id = self.env['res.country.state'].search([('name','=ilike',state),('country_id','=',country_id.id)], limit=1) if state else False
                    if state_id:
                        break
                    else:
                        state = state[1:]
                    ...
                    for local in oEnSUNAT.LoacalesAnexos:
                        ...
                        for i in range(1, len(state)):
                            ...
                            if state_id:
                                ...
                                state = state[1:]
                        city_id  = self.env['res.city'].search([('name','like',city),('state_id', '=', state_id.id),('country_id','=',country_id.id)], limit=1) if city else False
                        district_id = self.env['l10n_pe.res.city.district'].search([('name','like',district),('city_id','=',city_id.id)], limit=1) if district else False
